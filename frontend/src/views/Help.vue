<template>
  <div class="help-page">
      ...
      <el-collapse accordion>
        <el-collapse-item title="🔑 登录" name="login">
          ...
          <el-alert type="info" :closable="false" style="margin-top:10px">
            <template #title>
              <span>初始账号：管理员 <b>admin / admin123</b>，普通用户 <b>zhangsan / user123</b></span>
            </template>
          </el-alert>
        </el-collapse-item>
    ...
    <el-card class="section-card" id="section-points">
      ...
      <p class="desc-text">流水记录列：<b>类型、能量（负数红色）、原因、舰长寄语、发放人、时间</b></p>
    </el-card>
    ...
    <el-card class="section-card" id="section-admin">
      ...
      <el-collapse accordion>
        <el-collapse-item title="8.1 用户管理" name="admin-users">
          <p>路径：管理中心 → 用户管理。可新增、编辑、重置密码。支持用户名/姓名关键词搜索。</p>
        </el-collapse-item>
        <el-collapse-item title="8.2 发放星辰币" name="admin-thumbs">
          ...
          <ol class="step-list">
            <li>选择目标用户</li>
            <li>选择星辰币类型（发放/扣除）</li>
            <li>填写原因（必填），可填「舰长寄语」</li>
            <li>点击「发放」完成操作</li>
          </ol>
        </el-collapse-item>
        <el-collapse-item title="8.3 商品管理" name="admin-products">
        ...
        <el-collapse-item title="8.4 兑换管理" name="admin-exchanges">
          <p>查看所有用户兑换记录。仅对「已完成」状态可执行取消操作，取消后能量全额退回、库存恢复。</p>
        </el-collapse-item>
        <el-collapse-item title="8.5 心愿审核" name="admin-wishlists">
...
const thumbTypeData = [
  { type: '单星辰币', points: '+1', desc: '普通表扬' },
  { type: '双星辰币', points: '+5', desc: '优秀表现' },
  { type: '红牌警告', points: '−1', desc: '扣除惩罚（可用能量最低为0）' },
]
